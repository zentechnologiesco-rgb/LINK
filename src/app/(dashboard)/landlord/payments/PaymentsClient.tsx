...
import { AssignTenantWizard } from "@/components/properties/AssignTenantWizard"
import type { PropertyWithLease } from "@/components/properties/AssignTenantWizard"

type PaymentMethod = 'cash' | 'bank_transfer' | 'eft'
...
export function PaymentsClient({ payments, stats, deposits, properties }: PaymentsClientProps) {
    ...
    const handleRecordPayment = async () => {
        ...
        try {
            await recordPayment({
                paymentId: selectedPayment._id || selectedPayment.id as Id<"payments">,
                paymentMethod,
            ...
            setPaymentNotes("")
            // router.refresh() // Convex updates are realtime, no need to refresh unless props are from server
        } catch (error) {
    ...
    const handleDepositAction = async () => {
        ...
        setIsLoading(true)
        try {
            const depositId = selectedDeposit._id || selectedDeposit.id as Id<"deposits">
            ...
            setDeductionReason("")
            setForfeitReason("")
            // router.refresh() // Convex updates are realtime
        } catch (error) {
                                                    ...
                                                    <TableRow key={payment.id}>
                                                        <TableCell>
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <div>
                                                                    <div className="font-medium">{payment.lease?.tenant?.fullName || 'Unknown'}</div>
                    ...
                    <TabsContent value="deposits" className="space-y-4">
                                        ...
                                        {pendingDeposits.map((deposit) => (
                                            <div key={deposit.id} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-4 border rounded-xl">
                                                <div className="space-y-1">
                                                    <div className="flex items-center gap-2">
                                                        <User className="h-4 w-4 text-muted-foreground" strokeWidth={1.5} />
                                                        <span className="font-medium">{deposit.tenant?.fullName}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                        ...
                        <Card className="gap-0 py-0">
                            <CardHeader className="border-b px-4 sm:px-6 py-4">
                                ...
                                <CardDescription>Security deposits held by LINK on your behalf</CardDescription>
                            </CardHeader>
                            <CardContent className="px-4 sm:px-6 py-6">
                                            ...
                                            <TableBody>
                                                {completedDeposits.map((deposit) => (
                                                    <TableRow key={deposit.id}>
                                                        <TableCell>{deposit.tenant?.fullName}</TableCell>
                            ...
                            <div className="space-y-4">
                                <div className="p-4 bg-muted/30 border rounded-xl">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <p className="font-medium">{selectedPayment.lease?.tenant?.fullName}</p>
                        ...
                        {selectedDeposit && (
                            <div className="space-y-4">
                                <div className="p-4 bg-muted/30 border rounded-xl">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <p className="font-medium">{selectedDeposit.tenant?.fullName}</p>
