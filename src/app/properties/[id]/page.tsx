...
import { useState, useEffect, useRef, use, useCallback, TouchEvent, useMemo } from "react"
...
function getUnitInventoryFilter(unit: NonNullable<PropertyDetails["units"]>[number]): UnitInventoryFilter {
    if (unit.isAvailable || unit.occupancyStatus === 'vacant') return 'available'
...
function UnitInventorySection({ property, isOwner }: { property: PropertyDetails; isOwner: boolean }) {
    ...
    const sortedUnits = useMemo(
        () =>
            [...visibleUnits].sort((left, right) => {
                ...
                if (left.priceNad !== right.priceNad) return left.priceNad - right.priceNad
    ...
    const unitCounts = useMemo(
        () =>
            sortedUnits.reduce(
                (acc, unit) => {
                    acc.all += 1
                    acc[getUnitInventoryFilter(unit)] += 1
                    return acc
                },
    ...
    const preferredFilter: UnitInventoryFilter = !isOwner && unitCounts.available > 0 ? 'available' : 'all'
    const [activeFilter, setActiveFilter] = useState<UnitInventoryFilter>(preferredFilter)
    ...
    const filteredUnits = useMemo(() => {
        if (selectedFilter === 'all') return sortedUnits
    ...
    if (sortedUnits.length <= 1 && property.listingType === 'single_home') return null
                ...
                <main>
                    ...
                    <div className="px-5 pt-5 pb-8">
                        ...
                        <UnitInventorySection
                            key={`mobile-${property.id}-${isOwner ? 'owner' : 'viewer'}`}
                            property={property}
                            isOwner={isOwner}
                        />
                ...
                <main className="max-w-[1200px] mx-auto pt-24 px-10">
                    ...
                    <div className="flex gap-16 mt-10">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <UnitInventorySection
                                key={`desktop-${property.id}-${isOwner ? 'owner' : 'viewer'}`}
                                property={property}
                                isOwner={isOwner}
                            />
